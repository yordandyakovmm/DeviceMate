...
namespace DeviceMate.Objects.Helpers
{
	public static class StringFieldNameSortingSupport
	{
        ...
        public static IQueryable<TEntity> Order<TEntity>(this IQueryable<TEntity> queryable, string expression, string column, int direction, Func<IQueryable<TEntity>> defaultSortOperation) where TEntity : class
        {
            if (!string.IsNullOrWhiteSpace(expression))
            {
                queryable = queryable.OrderUsingSortExpression(expression);
            }
            else if (!string.IsNullOrWhiteSpace(column) && direction != 0)
            {
                queryable = direction > 0 ? queryable.OrderBy(column) : queryable.OrderByDescending(column);
            }
            else
            {
                queryable = defaultSortOperation();
            }

            return queryable;
        }

        public static IQueryable<TEntity> Paginate<TEntity>(this IQueryable<TEntity> queryable, int page, int pageSize) where TEntity : class
        {
            return queryable.Skip(pageSize * (page - 1)).Take(pageSize);
        }
